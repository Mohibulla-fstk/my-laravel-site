...
// Route::prefix('/mannage')->name('coupon.')->group(function () {
...
// Frontend AJAX apply/remove
Route::post('/coupon/apply', [CouponController::class, 'apply'])->name('coupon.apply');
Route::post('/coupon/remove', [CouponController::class, 'remove'])->name('coupon.remove');

Route::get('/search-top-products', [HomeController::class, 'getSearchTopProducts']);

Route::delete('/admin/childcategories/{childcategory}', [ChildcategoryController::class, 'destroy'])->name('admin.childcategories.destroy');
Route::prefix('admin')->name('admin.')->group(function () {
    Route::delete('/childcategories/{childcategory}', [ChildcategoryController::class, 'destroy'])
        ->name('childcategories.destroy');

});

Route::get('/update-lat-lng', [App\Http\Controllers\ContactController::class, 'updateLatLng']);

Route::get('/myfashionpanel', [LoginController::class, 'showLoginForm'])->name('login');
Route::post('/myfashionpanel', [LoginController::class, 'login']);

// Show OTP form
Route::get('/password/otp', [ForgotPasswordController::class, 'showOtpForm'])
    ->name('password.otp.form');

// Send OTP
Route::post('/password/otp/send', [ForgotPasswordController::class, 'sendOtp'])
...
// Reset password with OTP (submit new password)
Route::post('/password/otp/reset', [ForgotPasswordController::class, 'resetWithOtp'])
    ->name('password.otp');
Route::get('/otp-expiry', [ForgotPasswordController::class, 'getOtpExpiry'])->name('password.otp.get_expiry');
Route::post('/otp-resend', [ForgotPasswordController::class, 'resendOtp'])->name('password.otp.resend');
Route::post('/otp/validate-ajax', [ForgotPasswordController::class, 'validateOtpAjax'])
     ->name('password.otp.validate_ajax');
...
Route::get('category', [FrontendController::class, 'allCategories'])->name('pages.all');
// Cart Quantity Update
Route::get('/cart/increment/{rowId}', [CartController::class, 'increment'])->name('cart.increment');
Route::get('/cart/decrement/{rowId}', [CartController::class, 'decrement'])->name('cart.decrement');

// Cart Remove
Route::get('/cart/remove/{rowId}', [CartController::class, 'remove'])->name('cart.remove');
Route::get('/cart/menu', [CartController::class, 'menu'])->name('cart.menu');
...
Route::get('/shop', [App\Http\Controllers\Frontend\ShoppingController::class, 'showProducts'])->name('shop');
...
Route::prefix('admin')->group(function () {
    ...
    // Status toggle
    Route::post('/collections/active', [CollectionController::class, 'active'])
        ->name('collections.active');

    Route::post('/collections/inactive', [CollectionController::class, 'inactive'])
    ...
    // Delete
    Route::post('/collections/destroy', [CollectionController::class, 'destroy'])
        ->name('collections.destroy');

    // Collection items (category/sub/child add-remove)
    Route::post('/collections/add-item', [CollectionController::class, 'addItem'])
        ->name('collections.addItem');

    Route::post('/collections/remove-item', [CollectionController::class, 'removeItem'])
        ->name('collections.removeItem');

    Route::get('/collections/{id}/items', [CollectionController::class, 'manageItems'])
    ->name('collections.items');
    
    Route::get('/collections/edit/{id}', [CollectionController::class, 'edit'])->name('collections.edit');
    Route::post('/collections/update-badge', [CollectionController::class, 'updateBadge'])
    ->name('collections.updateBadge');

Route::put('/collections/update/{id}', [CollectionController::class, 'update'])->name('collections.update');
...
Route::prefix('admin')->middleware(['auth'])->group(function () {
    ...
    Route::get('/all-categories',
        [App\Http\Controllers\Admin\AllCategoryController::class, 'index']
    )->name('allCategories.index');

    Route::post('/all-categories/status-update',
        [App\Http\Controllers\Admin\AllCategoryController::class, 'statusUpdate']
    )->name('all.categories.status');

    Route::post('/all-categories/sort',
        [App\Http\Controllers\Admin\AllCategoryController::class, 'updateSort']
    )->name('allCategories.sort'); // 🔥 clean name, no admin prefix
    Route::post('/admin/update-badge', [AllCategoryController::class, 'updateBadge'])
...
Route::get('/check-user-status', [App\Http\Controllers\Auth\LoginController::class, 'checkStatus'])
...
Route::get('/notifications/fetch', function() {
    ...
    // Per-user is_read + deleted check
    $notifications->transform(function($n){
        ...
        $n->is_read = $record->is_read ?? false;
        $n->deleted = isset($record->deleted_at) && $record->deleted_at != null;
...
Route::get('/notifications/count', function() {
    ...
    foreach($notifications as $n){
        $read = DB::table('notification_user')
                    ->where('notification_id', $n->id)
                    ->where('user_id', Auth::id())
                    ->value('is_read') ?? false;
        if(!$read) $unreadCount++;
...
Route::get('/admin/notifications/fetch', function () {
    $userId = auth()->id();
    ...
    $notifications->getCollection()->transform(function ($n) use ($userId) {
        $record = DB::table('notification_user')
            ->where('notification_id', $n->id)
        ...
        $n->is_read = $record->is_read ?? false;
        $n->deleted = $record && $record->deleted_at ? true : false;

        return $n;
    });

    $filtered = $notifications->getCollection()
        ->filter(fn ($n) => !$n->deleted)
        ->values();

    // 🔥 unread count
    $unread = $filtered->where('is_read', false)->count();
    ...
    Route::get('/combo/{slug}', [ComboController::class, 'details'])->name('combo.show');
    Route::get('quick-view/{slug}', [FrontendController::class, 'quickView'])->name('quickview');
    Route::get('quick-view-combo/{slug}', [FrontendController::class, 'quickViewCombo'])
        ->name('quickviewcombo');
    ...
    Route::post('cart/combo', [ShoppingController::class, 'cart_store_combo'])->name('cart.combo');
    ...
    Route::get('/cart/decrement/{rowId}', [CartController::class, 'decrement'])->name('cart.decrement');
    Route::get('/cart/remove/{rowId}', [CartController::class, 'remove'])->name('cart.remove');
    Route::get('/cart/menu', [ShoppingController::class, 'cart_menu'])
    ->name('cart.menu');
...
Route::group(['prefix' => 'customer', 'namespace' => 'Frontend', 'middleware' => ['ipcheck', 'check_refer']], function () {
    ...
    Route::post('/post/combo-review', [ReviewController::class, 'storeComboReview'])->name('customer.combo.review');
...
Route::group(['namespace' => 'Admin', 'middleware' => ['auth', 'lock', 'check_refer'], 'prefix' => 'admin'], function () {
    ...
    Route::get('users/alluser', [UserController::class, 'showAllUsers'])->name('users.alluser');
    ...
    Route::post('users/update-activity', [UserController::class, 'updateActivity'])->name('users.updateActivity');
    Route::get('users/status', [UserController::class, 'fetchStatus'])->name('users.status');
    ...
    Route::post('combos/store', [ComboController::class, 'store'])->name('combos.store');
    ...
    Route::get('/combo/image/{id}', [ComboController::class, 'destroyImage'])->name('combo.image.destroy');
     Route::post('combos/inactive', [ComboController::class, 'inactive'])->name('combo.inactive');
    Route::post('combos/active', [ComboController::class, 'active'])->name('combo.active');
    ...
    // Coupon 
    Route::get('coupon/mannage', [CouponController::class, 'index'])->name('coupon.index');
    ...
    Route::post('coupon/active', [CouponController::class, 'active'])->name('coupon.active');
    Route::post('coupon/inactive', [CouponController::class, 'inactive'])->name('coupon.inactive');
    // product
    ...
    Route::post('order/cart-product-discount', [OrderController::class, 'product_discount'])->name('admin.order.product_discount');
    ...
    Route::get('order/cart-clear/{order_id}', [OrderController::class, 'cart_clear'])->name('admin.order.cart_clear');
    ...
    Route::get('customer/manage', [CustomerManageController::class, 'index'])->name('customers.manage'); // unique name
