...
<div class="container-fluid">
   ...
   <div class="row">
    ...
    <div class="form-group mb-3">
        <label for="product_id" class="form-label">Product / Combo *</label>
        <select class="form-control select2-multiple" name="reviewable_id" required>
    <option value="">Select..</option>
    
    {{-- Products --}}
    @foreach($products as $product)
        <option value="{{ $product->id }}" 
            {{ isset($edit_data) && $edit_data->product_id == $product->id ? 'selected' : '' }}>
            {{ $product->name }}
        </option>
    @endforeach
    ...
    @foreach($combos as $combo)
        <option value="{{ $combo->id }}" 
            {{ isset($edit_data) && $edit_data->combo_id == $combo->id ? 'selected' : '' }}>
            {{ $combo->name }}
        </option>
    @endforeach
</select>
