      ...
      <div class="col-12">
        <div class="page-title-box">
          <div class="page-title-right">
            <a href="{{route('admin.order.create')}}" class="btn btn-danger rounded-pill"><i class="fe-shopping-cart"></i>
              Add New</a>
          </div>
            ...
            <div class="row">
              <div class="col-sm-8">
                <ul class="action2-btn">
                  <li><a data-bs-toggle="modal" data-bs-target="#asignUser" class="btn rounded-pill btn-success"><i
                        class="fe-plus"></i> Assign User</a></li>
                  <li><a data-bs-toggle="modal" data-bs-target="#changeStatus" class="btn rounded-pill btn-primary"><i
                        class="fe-plus"></i> Change Status</a></li>
                  <li><a href="{{route('admin.order.bulk_destroy')}}" class="btn rounded-pill btn-danger order_delete"><i
                        class="fe-plus"></i> Delete All</a></li>
                  <li><a href="{{route('admin.order.order_print')}}"
                      class="btn rounded-pill btn-info multi_order_print"><i class="fe-printer"></i> Print</a></li>
                  @if($steadfast)
                    <li><a href="{{route('admin.bulk_courier', 'steadfast')}}?status=5"
                        class="btn rounded-pill btn-info multi_order_courier"><i class="fe-truck"></i> Steadfast</a></li>
                  @endif
                  <li><a data-bs-toggle="modal" data-bs-target="#pathao" class="btn rounded-pill btn-info"><i
                        class="fe-truck"></i> pathao</a></li>
                ...
                <tr>
                  ...
                  <td>
                    <div class="button-list custom-btn-list">
                      <a href="{{route('admin.order.invoice', ['invoice_id' => $value->invoice_id])}}" title="Invoice"><i
                          class="fe-eye"></i></a>
                      <a href="{{route('admin.order.process', ['invoice_id' => $value->invoice_id])}}" title="Process"><i
                          class="fe-settings"></i></a>
                      <a href="{{route('admin.order.edit', ['invoice_id' => $value->invoice_id])}}" title="Edit"><i
                          class="fe-edit"></i></a>
                  ...
                  <td>
                    @foreach($value->details as $detail)
                      @if($detail->combo_id)
                        Combo Product (ID: {{ $detail->combo_id }})<br>
                      @elseif($detail->product_id)
                        Regular Product (ID: {{ $detail->product_id }})<br>
                      @else
                        N/A<br>
                      @endif
                    @endforeach
                  ...
                  <td>{{date('d-m-Y', strtotime($value->updated_at))}}<br>{{date('h:i:s a', strtotime($value->updated_at))}}
                  </td>
                  ...
                  <td><a target="_blank" style="text-decoration: underline"
                      href="https://greenviewit.com/check-fraud-customer">Fraud Customer Check</a></td>

                  {{-- Coupon Code --}}
                  {{-- Coupon Column --}}
                  <td>
                    @php
                      $couponDetail = $value->details->whereNotNull('coupon_name')->first();
                    @endphp

                    @if($couponDetail)
                      {{ $couponDetail->coupon_name }} (-৳{{ number_format($couponDetail->coupon_discount_amount ?? 0, 2) }})
                    @else
                      -
                    @endif
                  </td>
                  ...
                  <td>
                    @php
                      ...
                      $finalAmount = $value->amount; // ডিসকাউন্ট বাদ দিয়ে ফাইনাল টাকা
                      $originalAmount = $finalAmount + $discount; // ডিসকাউন্টের আগে দাম
                    @endphp

                    @if($discount > 0)
                      <del>৳{{ number_format($originalAmount, 2) }}</del><br>
                      <strong>৳{{ number_format($finalAmount, 2) }}</strong><br>
                      <small>(-৳{{ number_format($discount, 2) }})</small>
                    @else
