            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{route('banner_category.create')}}" class="btn btn-primary rounded-pill">Create</a>
                    </div>
                            ...
                            <tbody>
                                @foreach($data as $key => $value)
                                    <tr>
                                        ...
                                        <td>@if($value->text_field == 1)<span
                                            class="badge bg-soft-success text-success">Enable</span>
                                        @else <span class="badge bg-soft-danger text-danger">Disable</span> @endif</td>
                                        <td>
                                            <div class="button-list">
                                                @if($value->status == 1)
                                                    <form method="post" action="{{route('banner_category.inactive')}}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{route('banner_category.active')}}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{$value->id}}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <a href="{{route('banner_category.edit', $value->id)}}"
                                                    class="btn btn-xs btn-primary waves-effect waves-light"><i
                                                        class="fe-edit-1"></i></a>
                                                <form method="post" action="{{route('banner_category.destroy')}}"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
                                                    <button type="submit"
